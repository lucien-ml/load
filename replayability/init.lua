local load = function(asset)
	...
	return loadstring(game:HttpGet(string.format("https://raw.githubusercontent.com/lucien-ml/load/main/%s/load/%s", sl, asset)))()
end
...
-- File System
if not isfolder("replayability") then makefolder("replayability") end
if not isfolder("replayability/assets") then makefolder("replayability/assets") end
if not isfolder("replayability/saves") then makefolder("replayability/saves") end
...
local TempPath = "replayability/saves/ReplayTemp.lua"
local PlaceId = tostring(game.PlaceId) -- tostring in order to access the files folder
local Folder = isfolder("replayability/saves/" .. PlaceId) or makefolder("replayability/saves/" .. PlaceId)
local CurrentSavePath = "replayability/saves/" .. tostring(PlaceId).. "\\Testing.lua"
...
if syn then  syn.protect_gui(CursorGui) end
		...
		wait()
			...
			if not ReadTable then 
				local Success, Return = pcall(function()
					return HttpService:JSONDecode(readfile("replayability/saves/" .. PlaceId.. "\\Testing.lua") .. "1]")
				end)
				if not Success then 
					Reading = false 
...
game:GetService("GuiService").ErrorMessageChanged:Connect(function() FullAbort = true end)
getgenv().Ran = true
